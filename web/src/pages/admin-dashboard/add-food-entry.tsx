...
import { addFoodEntry } from "../../services/foodEntry";
import { CreateFoodEntryDto } from "../../types/FoodEntry";
...
export default function AddFoodEntry({}) {
  ...
  const handleAddFoodEntry = async (foodEntry: CreateFoodEntryDto) => {
    try {
      const dateTime = getFormattedDateTime(foodEntry.date, foodEntry.time);
      const responseData = await addFoodEntry({
        userId: Number(foodEntry.userId),
        name: foodEntry.name,
